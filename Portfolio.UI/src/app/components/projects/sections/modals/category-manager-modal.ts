    ...
    template: `
        <div *ngIf="show" class="modal-overlay" (click)="close.emit()">
            <div class="modal-content max-w-3xl" (click)="$event.stopPropagation()">
                <div class="p-6">
                    <h4 class="text-base font-black dark:text-white text-zinc-900 mb-4">Manage Categories</h4>

                    <div class="grid grid-cols-[1fr_1fr_auto] gap-2 mb-4">
                        <input [(ngModel)]="newCategoryName" placeholder="Category (EN)" (keyup.enter)="addCategory.emit({name: newCategoryName, name_Ar: newCategoryNameAr})"
                        ...
                        <input [(ngModel)]="newCategoryNameAr" placeholder="الفئة (AR)" dir="rtl" (keyup.enter)="addCategory.emit({name: newCategoryName, name_Ar: newCategoryNameAr})"
                            class="px-3 py-2 rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-800 text-sm text-zinc-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-red-500/30">
                        <button (click)="addCategory.emit({name: newCategoryName, name_Ar: newCategoryNameAr})" [disabled]="!newCategoryName.trim()"
                    ...
                    <div class="border border-zinc-200 dark:border-zinc-700 rounded-lg overflow-hidden mb-4">
                        <table class="w-full">
                            ...
                            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                                <tr *ngFor="let category of categories" class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
                                    <td class="px-4 py-2 text-sm text-zinc-900 dark:text-white">{{ category.name }}</td>
                                    <td class="px-4 py-2 text-sm text-zinc-900 dark:text-white text-right" dir="rtl">{{ category.name_Ar || '-' }}</td>
                                    <td class="px-4 py-2 text-center">
                                        <button (click)="removeCategory.emit(category.id)" class="text-red-600 hover:text-red-700 transition-colors">
                                            <lucide-icon [img]="DeleteIcon" class="w-4 h-4"></lucide-icon>
                                        </button>
                                    </td>
                                </tr>
                                <tr *ngIf="categories.length === 0">
                                    <td colspan="3" class="px-4 py-8 text-center text-sm text-zinc-500">No categories yet. Add one above.</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
...
export class CategoryManagerModalComponent {
    ...
    @Output() addCategory = new EventEmitter<{name: string, name_Ar?: string}>();
