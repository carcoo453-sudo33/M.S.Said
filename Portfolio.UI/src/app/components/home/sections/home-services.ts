...
@Component({
    ...
    <div *ngIf="deleteIndex !== null" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <div class="relative bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-2xl w-full max-w-sm p-6 text-center animate-modal-enter">
            ...
            <p class="text-sm text-zinc-500 mb-6">{{ 'common.deleteMessage' | translate }} <strong class="text-zinc-900 dark:text-white">{{ editList[deleteIndex!].title || 'this service' }}</strong>? {{ 'common.deleteWarning' | translate }}</p>
            <div class="flex items-center justify-center gap-3">
                <button (click)="deleteIndex = null"
                    class="px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest text-zinc-500 hover:text-zinc-900 dark:hover:text-white border border-zinc-200 dark:border-zinc-700 transition-all">{{ 'common.cancel' | translate }}</button>
...
export class HomeServicesComponent {
    ...
    get translatedServices(): ServiceEntry[] {
        return this.translationHelper.translateArray(this.services, ['title', 'description']);
    }
