...
@Component({
    ...
    <section *ngIf="project" class="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4 animate-fade-in-up" style="animation-delay: 0.1s">
        ...
        <div class="bg-zinc-900/40 border border-zinc-800 p-4 lg:p-6 rounded-xl lg:rounded-2xl space-y-3 lg:space-y-4 hover:bg-zinc-900 hover:border-zinc-700 transition-all">
            ...
            <div class="text-sm lg:text-lg xl:text-xl font-black italic uppercase text-white truncate">{{ project.language || ('projectDetails.specs.multipleLanguages' | translate) }}</div>
        </div>
        <!-- Duration -->
        <div class="bg-zinc-900/40 border border-zinc-800 p-4 lg:p-6 rounded-xl lg:rounded-2xl space-y-3 lg:space-y-4 hover:bg-zinc-900 hover:border-zinc-700 transition-all">
            <div class="flex items-center gap-2 lg:gap-3 text-red-600">
            ...
            <div class="text-sm lg:text-lg xl:text-xl font-black italic uppercase text-white truncate">{{ project.duration }}</div>
        </div>
        <!-- Architecture -->
        <div class="bg-zinc-900/40 border border-zinc-800 p-4 lg:p-6 rounded-xl lg:rounded-2xl space-y-3 lg:space-y-4 hover:bg-zinc-900 hover:border-zinc-700 transition-all">
            <div class="flex items-center gap-2 lg:gap-3 text-red-600">
            ...
            <div class="text-sm lg:text-lg xl:text-xl font-black italic uppercase text-white truncate">{{ project.architecture || ('projectDetails.specs.scalableArchitecture' | translate) }}</div>
        </div>
        <!-- Status -->
        <div class="bg-zinc-900/40 border border-zinc-800 p-4 lg:p-6 rounded-xl lg:rounded-2xl space-y-3 lg:space-y-4 hover:bg-zinc-900 hover:border-zinc-700 transition-all">
            <div class="flex items-center gap-2 lg:gap-3 text-red-600">
            ...
            <div class="text-sm lg:text-lg xl:text-xl font-black italic uppercase text-green-500">{{ project.status || ('projectDetails.specs.active' | translate) }}</div>
        </div>
    </section>
  `
})
