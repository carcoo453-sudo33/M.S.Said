    ...
    <ng-container *ngIf="project">
        ...
        <section class="space-y-10 animate-fade-in-up" style="animation-delay: 0.2s">
            <div class="relative group rounded-[3rem] overflow-hidden border border-zinc-900 bg-zinc-950 aspect-video shadow-2xl">
                ...
                <div class="absolute bottom-10 left-10 right-10 flex justify-between items-end opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                    <div class="bg-black/60 backdrop-blur-md px-6 py-3 rounded-2xl border border-white/10">
            ...
            <div class="flex gap-6 overflow-x-auto pb-4 no-scrollbar">
                <div *ngFor="let img of project.gallery; let i = index"
                    class="w-48 aspect-video rounded-2xl border border-zinc-900 overflow-hidden shrink-0 grayscale hover:grayscale-0 cursor-pointer transition-all hover:scale-105 active:scale-95">
        ...
        <section class="flex flex-wrap gap-6 animate-fade-in-up" style="animation-delay: 0.3s">
            <a [href]="project.demoUrl" target="_blank"
                class="bg-red-600 text-white px-12 py-6 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center gap-4 hover:bg-red-700 transition-all shadow-xl shadow-red-600/20">
                <lucide-icon [img]="RocketIcon" class="w-4 h-4"></lucide-icon> Live Demo
            </a>
            <a [href]="project.repoUrl" target="_blank"
                class="bg-zinc-950 border border-zinc-800 text-zinc-300 px-12 py-6 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center gap-4 hover:border-red-600/50 hover:text-white transition-all">
